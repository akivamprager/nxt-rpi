...
| 3 | Camera calibration + ArUco pose | Not started (needs a camera) |
...
73 tests pass across the Pi stack (`pi/tests/`), none requiring hardware.
...
python3 pi/tests/test_protocol.py && python3 pi/tests/test_robot.py \
  && python3 pi/tests/test_sim_world.py && python3 pi/tests/test_sim_world_integration.py \
  && python3 pi/tests/test_mapping.py && python3 pi/tests/test_mission.py
```
...
pi/                 Python — runs on the Raspberry Pi
  ...
  tests/            protocol, robot, mapping, exploration, and mission tests
